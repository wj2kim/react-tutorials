...
var divToggle = document.querySelector(".toggle"); // DOM 노드를 가르키는 값을 선언

var counter = document.querySelector("h1");
var btnIncrease = document.querySelector("#increase");
var btnDecrease = document.querySelector("#decrease"); // 프로그램 상태에 변화를 일으키는 것을 액션이라고 한다. ( 액션 이름은 문자열 형태 + 대문자 + 고유 해야 한다.)

var TOGGLE_SWITCH = "TOGGLE_SWITCH";
var INCREASE = "INCREASE";
var DECREASE = "DECREASE"; // 다음으로 이 액션 이름을 사용하여 액션 객체를 만드는 액션 생성 함수를 작성 한다. ( 액션 값은 반드시 type 값을 가지고 있어야 한다.)
...
var store = (0, _redux.createStore)(reducer); // render 함수 작성 (상태가 업데이트 될 때마다 호출 됨, 리액트의 render 함수와는 다르게 이미 html을 사용하여 만들어진 UI의 속성을 상태에 따라 변경해 줍니다.)
...
render();
store.subscribe(render); // 액션을 발생시키는 것을 디스패치라고 합니다. ( 액션 객체를 파라미터로 넣어줍니다.)
